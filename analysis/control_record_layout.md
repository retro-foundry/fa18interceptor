...
| `$02` | word | `$C0A2F0` requires `(word & $C080) == $C080` for its selected-record scheduler setup. Direct-core run060 analysis observes `$C1B630` toggle bit 15 through the raw-$20 arrestor-hook route; the independently required bit 7 has no identified producer. | `data/run060_root_header_scheduler_operands.md`; `toggle_arrestor_hook.asm` |
