...
| `$28-$2A` | signed bytes | bounded three-axis control lanes. `$C1B410` updates them from separate two-bit fields in `+$65`; `$C1342C` uses `+$28` as a signed table lane. In the initial run060 turn, `+$28=-2` selects the first pitch-like target while the other two are zero. | `update_three_axis_control_bytes.asm`; `routines/c1b410_update_three_axis_control_bytes.md`; `routines/c1342c_matrix_side_leaf.md` |
...
| `$65` | byte | packed control byte. Its bits 5:4, 7:6, and 3:2 drive the signed `+$28`, `+$29`, and `+$2A` lanes respectively; low bits are used by other control routes. The initial run060 turn uses `$21`, whose `$20` field decrements `+$28`. | `update_three_axis_control_bytes.asm`; `routines/c1b410_update_three_axis_control_bytes.md`; `data/run060_control_mask_turn_timeline.md` |
