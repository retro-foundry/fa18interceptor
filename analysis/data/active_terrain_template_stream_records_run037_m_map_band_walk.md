...
Authority: `build/run037_m_map_band_walk_trace/trace.jsonl` and its `slow.bin`. The stream address is captured at `$C1D442`; each listed source is captured before the `(A5)+` header read at `$C1D488`.
