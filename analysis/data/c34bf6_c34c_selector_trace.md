...
`$C34BF6` (`$0A00`), shifts it to `$000A0000`, and compares it with runtime
state at `$C4619C`.  A lower result branches to `$C202F2`, restores the saved
registers, returns `D0=1`, and makes no C34C submission.  The observed path
does not take that rejection branch and advances `A4` from `$C46184` to
`$C46228`.
...
proves a conditional reject gate for this layer, rather than an alternate
geometry selection: the other branch returns from the handler.  The trace
does **not** yet establish the meaning of `$0A00` or the units of the compared
runtime value; calling the gate a distance threshold or an LOD rule would
