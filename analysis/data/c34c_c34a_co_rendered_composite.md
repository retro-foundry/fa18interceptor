...
Classification: **shared static template transform with spatially separate renderer instances**.
...
This makes a common composite renderer pass trace-proven. The subsequent [dual-lane source trace](c351_shared_dual_lane_flight_object.md) proves that both layers derive from the same `$C351xx` input stream. However, their captured coordinates are [spatially separated](c34a_c34c_frame12000_instance_separation.md), rejecting the earlier forward/nose-detail reading. Export must retain the two static face-record layers and procedural transforms as separate instances rather than collapsing them into an asserted immutable mesh.
