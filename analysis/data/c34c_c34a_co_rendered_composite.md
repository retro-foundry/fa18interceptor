...
Classification: **shared static-source transform with separate renderer face layers; procedural flight-object model candidate**.
...
This makes a common composite renderer pass trace-proven. The subsequent [dual-lane source trace](c351_shared_dual_lane_flight_object.md) proves that both layers derive from the same `$C351xx` input stream. It is compatible with the C34C detail being a forward/nose-like or other supplemental part of the aircraft-like C34A family. Export still must retain the two static face-record layers and procedural transforms rather than collapsing the result into an asserted immutable mesh.
