...
Classification: **trace-proven static vertex transform followed by renderer polygon contexts; user-identified aircraft-carrier deck/island candidate**.
...
The source set is substantially larger-scale than the `$C3515E` aircraft candidate. The user identified the silhouette as an **aircraft carrier, showing deck and island/tower only**. The ten-face pre-cull sheet strengthens that assessment: its X-Y view is a long deck-like plan with a small raised side structure, and its X-Z view is a thin deck profile. This remains a user visual identification tied to the trace-proven source-to-face chain, not a claim that every carrier feature has been recovered.

[The static-coordinate topology candidate](../plots/c39d2a_c3925_static_topology_candidate.svg) binds the ten observed face-offset lists directly to the 43 traced `$C39D2A` triples. It retains only those renderer-observed edges and makes no extra deck, hull, or island connections.
