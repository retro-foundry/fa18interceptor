...
| `$C3B720/$C3B6B0` | five immutable local triples and four traced triangular sides | exact map placement `$C4F0FA=(368,0,-144)` -> `$C3B6A6/$C3B6AE` -> `$C1F4AC` -> `$C3B6B0` -> four bounded `$C2FF48` primitives | exportable partial local component plus a separately retained observed placement/control instance; [instance asset](c3b6a6_bounded_instance_asset.json) preserves both spaces and explicitly excludes an unproven global-vertex composition |
