...
| `$C45BEA` | 5 unique pre-cull faces, 26 polygon edges at the Golden Gate checkpoint; alternate display-record base, with sampled words differing across checkpoints | plane-/missile-like silhouette, but **runtime-generated display record rather than a static model source** | [evidence](c45bea_runtime_display_record.md) Â· [pre-cull sheet](../plots/golden_gate_c45bea_preclip_face_sheet.png) |
| `$C3A94E` | 6 unique pre-cull faces, 24 polygon edges at frame 12600; distinct face family reached before culling | unidentified compact polyhedral face family | [pre-cull sheet](../plots/frame12600_c3a94e_preclip_face_sheet.png) |

The catalogue deliberately keeps the last entries separate from the static-model candidates. `$C45BEA` is especially important: `$C0D74A` selects it as an alternate display-record base and the matrix-update route writes that cache, so its aircraft-like silhouette is useful for recognising what is on screen but cannot be exported as immutable model data. `$C38F98` and `$C3A94E` remain drawable renderer families whose upstream static source/ownership is not yet established.
