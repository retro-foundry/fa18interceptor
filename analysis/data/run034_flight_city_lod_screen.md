...
The reproducible grayscale measurement now supplies that mask target without
assigning it an object name. In viewport `x=40..679, y=18..159`, grayscale
levels `$33..$88` form two principal connected regions: 6,922 pixels at
`x=212..505, y=100..159`, and 1,242 pixels at `x=68..209, y=63..114`.
The ignored report is `build/run034_8000_grayscale_regions.json`, generated by
`scripts/measure_grayscale_viewport_regions.py`. These are raster regions, not
yet city, terrain, or primitive ownership claims.

A normal, non-debugger-stepped frame-8,000 trace provides the first same-frame
primitive lead. It contains no `$C2FF48` finalized-polygon entry, so the grey
surface is persistent display content in that frame. Its sole scene `$C2FA7E`
line has mutable/negative `A5=$FFFFFFE8` and renderer endpoints
`(30,69)->(260,179)`, converting to screenshot `(100,85)->(560,195)`. That
segment crosses the 6,922-pixel grayscale region. This is a line-to-grey-raster
spatial correlation, not source or object ownership: the context is mutable,
