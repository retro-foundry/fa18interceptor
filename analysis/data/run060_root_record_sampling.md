...
live formatter packet and the end-of-run 145 FT cockpit screen establish root
`+$18` as the selected cockpit altitude field; see
`run060_root_altitude_formatter.md`. The live packet itself takes an
unchanged-value cache path, so this remains scenario-backed rather than a
single-invocation glyph contract. Those promotions establish an
input-controlled record with an altitude field, not horizontal position or
orientation. The next player-state
