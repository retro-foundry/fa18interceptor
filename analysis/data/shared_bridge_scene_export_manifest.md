...
is the machine-readable handoff for the one local bridge component proven in
...
It also does not promote the component into a complete Golden Gate mesh—the
scene can reuse it across bridge instances and no unseen faces are inferred.
`$C3B720` is deliberately excluded after visual identification as a terrain /
mountain candidate rather than bridge structure.
