...
| ---: | --- | --- | --- |
| 1 | `$C1CC86/$C1CFA6 -> $C1EE14`, including `$C1ED3C/$C1ED4C` | Common conditional stage between placement descriptors, immutable triples, control streams, and both renderer outputs; appears in run037 and run041. | At each run041 5,750/6,000 mixed-family call, retain the descriptor selected at `$C1CC70`, indirect target, `$C45A36`/cursor publication, `$C1F4AC` source `A1`, and `$C1F6F8` stream `A5` in the **same** returned invocation. Compare with 5,000 and 6,250. |
| 2 | `$C1C5E0 -> $C45A78 -> $C2AA9C` | The publisher and map metric formula are byte-exact, but the transformed tuple's live context is not tied to physical range. This path feeds both M-map packet passes and most traced map polygons. | From the known `$C1C636` store, trace the tuple's selected `A2` record and transform inputs in the same replay that reaches `$C2AAD2`; compare packet/control choices at distinct map states. Keep projection depth distinct from physical distance. |
| 3 | `$C1CB14/$C1CB26 -> $C1CC86` placement loop | Selects which descriptor and callback enters priority 1; required for a record-to-renderer ownership join. | Capture one uninterrupted template-copy -> placement record -> descriptor -> returned `$C1EE14` frame -> primitive sequence. |
