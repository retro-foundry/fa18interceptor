...
| ---: | --- | --- | --- |
| 1 | Placement-cache builder -> `$C1CB14/$C1CB26` -> `$C1CC86/$C1CFA6` | The shared primary/alternate descriptor inputs are now tied to active mutable records, but their construction is still not joined to the later returned stage. This is the remaining producer boundary that can explain many descriptor calls at once. | Capture one uninterrupted template/workspace update -> `$C1DC1C-$C1E0B0` record build -> matching `$C1CC60` or `$C1CF6A` field publication -> returned `$C1EE14` (or `$C1ED3C/$C1ED4C`) call and primitive sequence. The concrete first target is `$C4E9C2`: run033 proves its `$C4264D -> $C4A970 -> $C4E9C2/$C22408` rebuild, but its in-window `$C1EE14` call precedes that rebuild. See `analysis/data/run033_c4e9c2_rebuild_pending_descriptor_consumer.md`. The run033 post-refresh `$C096CA` call is explicitly not that join. |
| 2 | `$C1CC86/$C1CFA6 -> $C1EE14`, including `$C1ED3C/$C1ED4C` | Common conditional stage between placement descriptors, immutable triples, control streams, and both renderer outputs. | The frame-5,750/6,000 fixtures now join selected fields, stage target, cursor, immutable source, walker, and return. Extend only to a different active-record construction event or an unrepresented branch; do not repeat a static call census. |
