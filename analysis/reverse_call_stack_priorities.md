...
| ---: | --- | --- | --- |
| 1 | Result-state writer before the shared postflight progression | The visible run060 selector-74 and run062 selector-63 writes are downstream timed stages. Run062 GUI frame 2,000 begins with `$C1820C=$C10DAE`; its directly observed gate-positive path installs `$C11788`, and the slot changes at replay frame 133. `$C11788` subsequently schedules `$C118A0`, which queues 63. Thus neither callback transition is itself the pass/fail decision. | Work backward from the earliest run060/run062 state difference that selects or prepares `$C10DAE`'s record/control fields. Retain `$C46184`, `$C46200`, `$C4578C`, `$C45785`, callback slot, and timer values with their writer PCs. Keep countdown expiry, crash logic, persistence, and the pass/fail predicate as separate claims until a controlling branch is traced. |
