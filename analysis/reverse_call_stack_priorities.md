...
| ---: | --- | --- | --- |
| 1 | Result-state writer before the shared postflight progression | The visible run060 selector-74 and run062 selector-63 writes remain downstream timed stages. Run060 now has a native checkpoint chain from scheduler eligibility through `$EF` and `$C11958`; its final `+$6E` gate transition is locally reproduced from native frame 9,208 at `$C1486A`. Run062's negative candidate return instead sets record bits through `$C26102/$C26178` before its failure-side callback progression. | Work backward from the **native producers** of run060 `$C46184+1` and `+$2` gate operands, then compare their timing/value sources with run062's record transition. Retain `$C46200`, `$C4578C`, `$C45785`, callback slot, and timer values with writer PCs. Keep countdown expiry, crash logic, persistence, and the pass/fail predicate as separate claims until a controlling branch is traced. |
