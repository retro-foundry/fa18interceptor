...
| ---: | --- | --- | --- |
| 1 | Result-state writer before the shared postflight progression | The visible run060 selector-74 and run062 selector-63 writes remain downstream timed stages. Run060 now has a complete local scheduler chain: mode-9 `$C09E78` calls `$C0A2F0`; its observed `$C46184` gate writes selector/phase/countdown state; `$C0F5F8` expires and dispatches `$C11078`; that callback installs `$C110A4`, which produces selector 74. Run062's negative candidate return instead sets record bits through `$C26102/$C26178` before its failure-side callback progression. | Work backward from the **producers** of run060 `$C46184+1`, `+$2`, and `+$6E` gate operands, then compare their timing/value sources with run062's record transition. Retain `$C46200`, `$C4578C`, `$C45785`, callback slot, and timer values with writer PCs. Keep countdown expiry, crash logic, persistence, and the pass/fail predicate as separate claims until a controlling branch is traced. |
