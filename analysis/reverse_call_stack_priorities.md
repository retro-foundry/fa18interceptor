...
| ---: | --- | --- | --- |
| 1 | Static-template selector -> workspace copy -> `$C1DC1C-$C1E0B0` builder | The complete `$C4264D -> $C4A370 -> $C4E9C2/$C22408 -> $C1EE14 -> $C35598/$C35BAA` chain is now proved. The selector that supplies template entries is the remaining shared upstream boundary, capable of explaining many placement records rather than one descriptor call. | Trace the selection through `$C1D10C`, `$C1D3F4`, and `$C1D442` into a `$C1D488` copy while retaining the selected source pointer, workspace destination, and the downstream generated record. Start from the `$C4264D` fixture in `analysis/data/run033_c4e9c2_builder_to_descriptor_stage.md`; distinguish a selector rule from any physical-world interpretation. |
