...
| ---: | --- | --- | --- |
| 1 | Qualification-result state -> negative `$C45AD6` and postflight mode `$C458A6=9` | The run060 writer is now directly proved: `$C11186` stores selector 74 at `$C4574A`. The remaining shared upstream boundary is the result-state transition that makes `$C110A4` take that mode-9 queue route; it can explain success/failure message families without inventing a landing predicate. | From the frame-9,284 checkpoint, trace backward from `$C110A4`'s negative-delay/mode-9 inputs through their last writers, then compare the corresponding run062 failure state. Keep the result condition, crash logic, and persistence separate until a branch/consumer trace proves each. |
