...
| ---: | --- | --- | --- |
| 1 | Qualification-result producer -> `$C4574A` message-sequence head -> selector 74 | Deterministic run060 proves selector 74 produces `LANDING SUCCESSFUL`, but not the test or writer that activated it. Recovering that producer connects qualification result state to the shared message queue and its compositor, while also explaining sibling outcome messages without inventing a landing rule. | Rebase a CPU write watch for `$C4574A` against the actual replay-frame clock, retain the writer's call frame and queue contents, then trace its controlling result branch. The nominal GUI checkpoint frame 9,285 is not yet a usable replay-frame arm value. |
