...
The same three-frame trace has a direct writer for the band cells between
captured placement-builder passes.  `$C1D266` selects `$C411F0`, `$C1D272` saves that table-derived pointer,
...
cell rejection-marker reset, not coordinate population.  In the bounded trace
it follows the frame-1 builder activity and precedes the next builder entry at
