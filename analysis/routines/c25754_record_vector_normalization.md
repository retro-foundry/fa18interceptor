...
Byte-exact source is `source_amiga/observed/normalize_record_vector.asm`.
...
writes three words to `$C45A4C-$C45A50`. The observed input takes the negative
magnitude route and the scaling loop before returning with sign-extended
components in `D5-D7`.
