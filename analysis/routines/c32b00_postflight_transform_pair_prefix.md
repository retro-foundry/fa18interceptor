...
It reads two words from `A1` (the first becomes the renderer destination
offset; the second becomes a later mask component) and a byte from `A2`,
rejects a space byte or a derived row outside `[0,$28)`, adds the prepared
offsets, and turns the byte into a zero-based word-table index by subtracting
`$20`.
