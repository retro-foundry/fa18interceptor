# Library-vector wrapper at `$C53F9C`
...
vector. The observed target immediately transfers into Kickstart ROM, so the
vector is deliberately left unnamed. The complete `$C53F9C-$C53FAF` wrapper
is byte-exact source in
