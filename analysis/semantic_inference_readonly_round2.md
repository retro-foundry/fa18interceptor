...
| 3 | [`$C231A2`](routines/c231a2_paired_record_guard.md) | Behavioral paired-bank rejection prefilter | Shared by bank-9, bank-11, bank-13 update callers | Captured rejections only; success and `$C457BC` alternatives unassigned. |
| 4 | [`$C265E8`](routines/c265e8_flagged_slot_scan.md) | Behavioral 20-slot flag scan on no-match path | Feeds the parent flight-update true/false branch; table stride shared with `$C151CE` | All 20 tests and zero return observed in run060; match continuation unassigned. |
...
the latter needs its separate generated-coverage reconciliation. The next
highest-value questions are the `$C27456` candidate's world/state owner,
`$C231A2`'s unobserved success continuation, and the match route of
`$C265E8`; each needs a targeted call/return or producer-consumer trace
before stronger gameplay names are justified.
