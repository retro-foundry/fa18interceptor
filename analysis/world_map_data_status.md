...
| Is a static input known for that traversal? | Yes: the active cursor resolves to byte-stable, relocation-free segment 65 at `$C412EC`.  Its observed byte controls select workspace processing; no terrain-coordinate read from it is proven. | [band-walk contract](routines/c1d330_scene_workspace_band_walk.md) |
| Is any static terrain-scene input copied into those cells? | Yes.  The trace copies the verified, non-relocation-overlapped segment-66 sequence at `$C427C1` into `$C4B270`, which the following builder pass consumes.  It is a transformed control/template record, not yet a raw global-coordinate or mesh record. | [workspace template copy](routines/c1d442_workspace_cell_template_copy.md) |
