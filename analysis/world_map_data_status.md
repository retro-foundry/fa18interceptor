...
| Does `M` identify map data? | Partly. Raw `$37` reaches `$C1BF8C`; its now-completed 5,619-instruction command helper is transition/page-control work and does not execute the terrain template selector/copy path. The later map-display renderer now has a bounded transform/control index: `$C35BDE -> $C35BF0`, `$C35BAA -> $C35BB8`, `$C35BC2 -> $C35BD0`, `$C36220 -> $C36232` twice, and `$C3B720 -> $C3B73E`. `$C36220` is bounded to two visible line segments; `$C3B720` is bounded through `$C3B6B0` to both line and span primitives in the prepared map page. The five unique components are packaged as a qualified partial export. This is not a complete world-terrain extraction or a coastline-pixel mapping. | [`M` command contract](routines/c1bf8c_map_command.md), [partial export](data/run003_m_map_partial_geometry_export.md), [renderer census](data/run003_m_map_display_renderer_census.md), [line component](data/c36220_c36232_map_line_component.md), and [component boundary](data/c3b720_c3b6b0_static_component_boundary.md) |
