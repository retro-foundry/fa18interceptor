...
| Does the wide pass have its own depth-threshold control-stream selector? | Structurally, yes. `$C2AC1A` selects bases `$C2A0C2`, `$C2A072`, or a low-metric filter path at `$C2AC3E` around `$10000/$9000`; run035 appearance takes the low branch twice. This is a depth-driven control-stream mechanism, not yet a measured physical-distance or mesh-LOD result. | [wide directory initializer](routines/c2ab34_wide_map_packet_directory_setup.md) |
