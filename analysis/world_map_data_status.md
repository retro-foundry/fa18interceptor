...
| Does `M` identify map data? | Partly. Raw `$37` reaches `$C1BF8C`; its now-completed 5,619-instruction command helper is transition/page-control work and does not execute the terrain template selector/copy path. The later map-display renderer now has one bounded immutable geometry/control consumer: `$C3B720-$C3B73D` transforms to `$C48390`, then `$C3B73E -> $C3B6B0` produces both line and span primitives in the prepared map page. This is a partial local component, not a complete world-terrain extraction or a coastline-pixel mapping. | [`M` command contract](routines/c1bf8c_map_command.md), [renderer census](data/run003_m_map_display_renderer_census.md), and [component boundary](data/c3b720_c3b6b0_static_component_boundary.md) |
