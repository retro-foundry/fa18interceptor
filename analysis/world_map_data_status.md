...
| Do those substitutions reach renderer control? | Yes. Their differing repeated descriptor target occupies the `+8` field that `$C1CB74-$C1CCB6` writes to `$C45A36`; `$C1F6F8` consumes that pointer on the projection path. The selection changes renderer control streams, but remains distance-confounded. | [descriptor-control-stream dataflow](data/placement_refresh_window_comparison.md) |
