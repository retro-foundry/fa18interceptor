...
| Are there further static packet-header leads? | Yes, but only as candidates. An even-address grammar scan finds 71 header/inline/alternate structures; all 24 live headers are included, while 47 unexecuted candidates follow the same format. They are a targeted trace list, not promoted map packets, because coordinate payload can imitate the compact grammar. | [candidate header inventory](data/static_m_map_packet_streams.md#grammar-compatible-header-candidates) |
| Does the stable run037 M-map window execute candidate-only headers? | No, in the extended no-input sample. Across 300,000 stepped instructions, 56 `$C2AF40` visits repeat 23 already known headers and execute none of the 47 candidate-only addresses. This rejects them for that stable map draw, not for other map positions or modes. | [run037 long selector sample](data/run037_m_map_packet_long_window.md) |
| Does the run035 map-appearance variant execute candidate-only headers? | No, in its matching 300,000-instruction no-input sample. Forty visits split 20 inline/20 alternate selections over eight already known headers, with no candidate-only header. This strengthens depth-driven stream variation for an existing packet family, but does not establish flight-world LOD. | [run035 long selector sample](data/run035_m_map_packet_long_window.md) |
