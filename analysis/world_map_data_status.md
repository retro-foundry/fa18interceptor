...
| Does the wide pass have its own depth-threshold control-stream selector? | Yes. `$C2AC1A` selects bases `$C2A0C2`, `$C2A072`, or a low-metric filter path at `$C2AC3E` around `$10000/$9000`. Sealed run035 traces cover low/high outcomes; a paused-state `$A000` metric probe covers the middle base. This is a verified depth-driven control-stream mechanism, not yet measured physical-distance or mesh-LOD. | [wide directory initializer](routines/c2ab34_wide_map_packet_directory_setup.md), [stable branch sample](data/run035_m_map_stable_wide_depth_branches.md), and [middle-band probe](data/run035_m_map_stable_wide_depth_mid_probe.md) |
