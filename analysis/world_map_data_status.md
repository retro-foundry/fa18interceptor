...
| Is a selector boundary observed? | Locally. Setting the row-axis origin component to zero preserves group indices but suppresses the low-key stream set (16 to 4 streams) with no new wraparound stream. This is not a proven global map edge. | [zero-bin boundary probe](data/origin_selector_mutation_probe.md) |
