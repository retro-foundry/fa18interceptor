...
| Are the two selector inputs separable beyond a one-bin probe? | Yes, for bins 0--31 in the same bounded update packet. Every group-axis bin changes only `$C1D3F4`'s selector indices, and every row-axis bin changes only row keys; bins 1--30 alter the same 28/41 call positions. The bin-31 partial zero behavior is retained verbatim. This is a concrete directory-input map, not an absolute world-coordinate map. | [controlled axis sweep](data/origin_selector_axis_sweep_00_1f.md) |
...
| Does `M` visibly show an in-game map? | Yes. The sealed run003 `M` event changes the cockpit to a stable green/blue coastline-style grid display within 30 frames. Copper evidence identifies it as a 320x200, four-bitplane, double-buffered Chip-RAM display; its bitmap producer is still untraced, so the screen is a visual oracle rather than extracted terrain data. | [`M` visual probe](data/run003_m_map_visual_probe.md) |
