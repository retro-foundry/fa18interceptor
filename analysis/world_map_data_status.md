...
| What feeds the placement builder's workspace scan? | The same update phase first resets leading cell words to `$FFFF` at `$60`-byte stride, then `$C1D330-$C1D3E6` traverses `$C48390` in `$600`-byte bands and enters the builder.  This proves mutable-cell lifecycle, not the original source. | [band-walk contract](routines/c1d330_scene_workspace_band_walk.md) |
