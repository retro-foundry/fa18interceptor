...
| Does `M` identify map data? | Not yet. Raw `$37` reaches `$C1BF8C`; its now-completed 5,619-instruction command helper is transition/page-control work and does not execute the terrain template selector/copy path. The later map-display renderer still lacks a traced static asset consumer. | [`M` command contract](routines/c1bf8c_map_command.md) |
| Does `M` visibly show an in-game map? | Yes. The sealed run003 `M` event changes the cockpit to a stable green/blue coastline-style grid display within 30 frames. A controlled run035 end-of-flight `M` view shows the same coastline panned by `(142,36)` screen pixels with 98.8008% blue-raster agreement. Copper evidence identifies it as a 320x200, four-bitplane, double-buffered Chip-RAM display. The video-hash-matched stable map interval runs terrain-template selection and sends 103 copied static records to X/0/Z placement outputs; that is map-mode terrain dataflow, not yet a placement-to-coastline-pixel proof. Its bitmap producer is still untraced, so the screen remains a visual oracle rather than extracted terrain data. | [`M` visual probe](data/run003_m_map_visual_probe.md), [template-placement handoff](data/run003_m_map_template_placement_handoff.md), [pan comparison](data/run003_run035_m_map_pan_comparison.md), and [screenshots](visuals/run003_m_map_display.png) |
