...
| Is any actual 3D terrain-like geometry proven in M-map mode? | Yes, one compact reusable component. An active flat placement/template route reaches immutable `$C3B720-$C3B73D`, five local triples with four traced triangular sides under `$C3B6B0`; the M-map transition then submits its line/span primitives to the prepared map page. Its fifth local vertex has middle component `1024`, so this proves local 3D component geometry while **not** contradicting the separately sampled X/0/Z placement plane. It is not a complete terrain mesh, global elevation model, or named map feature. | [component boundary](data/c3b720_c3b6b0_static_component_boundary.md), [single-trace placement/control handoff](data/run037_m_map_placement_to_control_20f_handoff.md), and [3D inspection plot](plots/c3b720_shared_bridge_pylon_static_topology.png) |
| Does the same component execute in ordinary demonstration flight? | Yes. An event-faithful demo replay reaches descriptor `$C224D0` field `$C3B6A6`, publishes `$C3B6AE`, transforms `$C3B720`, and makes four `$C2FF48` calls under `$C3B6B0`. This expands scenario coverage to flight, but does not identify it as a named building or landmark, or establish global placement. | [demo component path](data/attract_demo_c3b6a6_renderer_path.md) |
| Does the longer demo-flight refresh draw more scene content? | Yes, as renderer activity only. A bounded ordinary 30-frame demo window makes 64 polygon and 88 line submissions under two repeatedly loaded control streams. It has no `$C1F4AC` immutable-triple transform in that interval, and lacks a simultaneous placement trace, so it does not establish buildings, landmarks, meshes, or instance ownership. | [demo refresh census](data/attract_demo_scene_refresh_renderer_census.md) |
| Does demo descriptor field `$C3B4F8` select either repeated demo stream? | Not in its first bounded post-store interval. The exact frame-139 `$C1CC70` store is followed for 12,000 instructions with no cursor publication, walker, transform, or primitive submission. This rejects a direct ownership join to `$C3924C` or `$C34A5E` in that execution only; it is not a global rejection or a landmark identification. | [demo control boundary](data/attract_demo_c3b4f8_control_boundary.md) |
| Does another demo descriptor reach a repeated scene primitive family? | Yes. At demo frame 516, descriptor field `$C44500` cursor-publishes and walker-loads `$C3924C`, followed by three `$C392A8` polygon and one line submissions. There is no immutable-source read in the bounded interval, so this is control-to-primitive execution only, not a placed mesh or building identity. | [demo C44500 path](data/attract_demo_c44500_to_c392a8_control_path.md) |
| Does a later ordinary demo view prove another local-geometry component? | Yes. From the frame-3,600 checkpoint, descriptor `$C22870` field `$C3B5A6` publishes/loads `$C3B5B4`, transforms five local triples `$C3B628-$C3B645`, then reaches two `$C3B5B6` polygon submissions. It repeats four times in the 30-frame refresh. This is a direct descriptor-to-local-geometry-to-polygon sequence, but no flight placement record or feature identity is joined. | [later demo component path](data/attract_demo_c3b5a6_to_c3b5b6_component_path.md) |
| Does the other later-demo descriptor `$C44640` own that component? | No, in its first bounded continuation. `$C1CC70` stores `$C44640` from `$C22834`, then a later `$C3B5A6` store occurs before cursor publication or a walker; the latter, not `$C44640`, leads to the observed local geometry and polygons. This is execution-specific and does not reject another `$C44640` path. | [demo C44640 boundary](data/attract_demo_c44640_control_boundary.md) |
...
| Do run037 map placements reach the renderer control interface? | Yes. In one uninterrupted 20-frame M-map trace, six exact placement records supply descriptors whose `+8` fields pass through `$C1CC70`, `$C1EF10`, and `$C1F70E` into the matching `$C1F6F8` walker `A5` control pointer. This proves placement-to-control-stream execution, but not exclusive ownership of each later primitive, pixel, model, or terrain cell. | [single-trace handoff](data/run037_m_map_placement_to_control_20f_handoff.md) |
| Is another placed map component bounded through actual primitive emission? | Yes. Template `$C42707` yields placement `$C4EC02=(0,0,-328)`, descriptor field `$C35BD0`, and published stream `$C35BD2`; in the same trace it transforms `$C35BDE` and emits the decoded two-edge `$C35BD4` polyline. It is an exact placed component, but has no established semantic identity or static global vertices. | [placed polyline path](data/run037_c35bd0_placement_to_polyline_context.md) |
| Are all run037 walker components now bounded to placement and primitives? | Yes, for this 20-frame trace only. Its six walkers each have an exact placement/control/source chain and following bounded line or polygon output: one unknown polyline, two scenario-correlated red-line-family instances, two `$C36220` line instances, and one `$C3B720` filled component. This does not make the six records a complete world export. | [six-component inventory](data/run037_bounded_placed_component_inventory.md) |
| Does the Golden-Gate-correlated red-line family have exact map placements? | Yes, with a name boundary. `$C3559A` and `$C355D2` each have independent template/placement, descriptor/control, source-pair, and `$C2FA7E` line-emission chains in run037: respectively `(460,0,172)` and `(460,0,148)`. The family is verified as Golden Gate in run003/run033, but its run037 reuse does not establish a global landmark pin. | [placed red-line-family instances](data/run037_c355_red_line_family_placed_instances.md) |
| Does the verified Golden Gate M-map scenario itself have exact landmark placements? | Yes. In the run003 M-map trace, the `$C3559A` and `$C355D2` Golden Gate strokes independently follow template/placement, descriptor/control, source-pair, and line-emission paths at `(588,0,300)` and `(588,0,276)`. This proves its two observed landmark line instances, not a complete bridge mesh or global vertex export. | [run003 Golden Gate placed-line instances](data/run003_golden_gate_placed_line_instances.md) |
| Is a reproducible renderer pose available for those Golden Gate lines? | Yes. The run003 trace fixes each pair's local shift and live terms, while the saved `$C45BD8` matrix yields two exact view-space endpoints per red line. The result is retained as view-dependent output rather than recast as global bridge geometry. | [Golden Gate view-space pose](data/run003_golden_gate_view_pose.md) |
