...
| Does `M` identify map data? | Partly. Raw `$37` reaches `$C1BF8C`; its now-completed 5,619-instruction command helper is transition/page-control work and does not execute the terrain template selector/copy path. The later map-display renderer consumes static 3D control input, including `$C3B73E -> $C3B6B0`, but still lacks a traced immutable terrain-vertex/asset consumer. | [`M` command contract](routines/c1bf8c_map_command.md) and [renderer census](data/run003_m_map_display_renderer_census.md) |
