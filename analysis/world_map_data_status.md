...
| Do the verified Golden Gate strokes have exact map placements? | Yes. The `$C3559A` and `$C355D2` landmark strokes each have an independent template/placement, descriptor/control, source-pair, and `$C2FA7E` line-emission chain in the run037 trace: respectively `(460,0,172)` and `(460,0,148)`. This is landmark placement-to-line ownership, not a complete bridge mesh or global-vertex export. | [Golden Gate placed-line instances](data/run037_golden_gate_placed_line_instances.md) |
