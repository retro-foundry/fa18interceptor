...
| Can unrendered packet data be structurally separated from unknown bytes? | Partly. Twenty-four direct `$C2AF00` headers plus two additional `$C2AF40` selector-sampled headers expose 34 inline/alternate streams whose count/threshold grammar decodes to 489 pair records and `$FFFF` terminators, covering 31.43% of segment 68 as pair payload. These are statically reachable from observed headers, not evidence that every pair was rendered or is terrain. | [static reachable packet streams](data/static_m_map_packet_streams.md) |
| Are there further static packet-header leads? | Yes, but only as candidates. An even-address grammar scan finds 71 header/inline/alternate structures; 26 have live renderer evidence, while 45 unexecuted candidates follow the same format. They are a targeted trace list, not promoted map packets, because coordinate payload can imitate the compact grammar. | [candidate header inventory](data/static_m_map_packet_streams.md#grammar-compatible-header-candidates) |
...
| Are both selector row strides used in recorded map draws? | Yes. The frame-local mode at `-$3E(A6)` is zero for run003/run037 (16-byte rows) and non-zero for both run035 M-map traces (64-byte rows), at both observed directory bases where reached. This proves alternative packet-directory layouts are live, but does not establish physical-distance LOD because the captures differ in map transition/state. | [selector stride modes](data/m_map_selector_modes.md) |
