...
| Is any static terrain-scene input copied into those cells? | Yes.  The trace copies 37 static entries from verified segments 66--67 into cells; 22 are later read by the builder before the trace ends.  The `$C427C1 -> $C4B270` example is a transformed control/template record, not yet a raw global-coordinate or mesh record. | [copy inventory](data/workspace_template_copies.md) |
