...
def main() -> None:
    ...
    placement_payload = json.loads(args.placements.read_text(encoding="utf-8"))
    placement_rows = placement_payload["copies"]
    trace_authority = str(args.trace.relative_to(ROOT)).replace("\\", "/")
    same_trace = placement_payload.get("authority", {}).get("trace") == trace_authority
    ...
    for descriptor, instances in placements_by_descriptor.items():
        unique = {(item["static_source"], item["runtime_placement_record"], tuple(item["coordinates"])): item
                  for item in instances}
        placements_by_descriptor[descriptor] = list(unique.values())
    writes = []
    for row in trace:
    ...
    payload = {
        "classification": ("single_trace_template_placement_to_control_stream_ownership_not_yet_per_primitive_mesh_or_pixel_ownership"
                           if same_trace else
                           "cross_capture_scenario_correlated_template_placement_to_control_stream_ownership_not_yet_single_execution_mesh_or_pixel_ownership"),
    ...
    lines = [
        ...
        (f"**{payload['summary']['unique_descriptors']}** distinct descriptors. Every observed descriptor is also present in this trace's template-placement inventory. `$C1CC70` reads each descriptor's `+8` longword and writes it to `$C45A36`, the input that `$C1F6F8` loads as its control-stream pointer."
         if same_trace else
        ...
        "",
        (f"The same trace enters `$C1F6F8` **{payload['summary']['walker_entries']}** times. For each entry, the exact earlier placement record supplies the descriptor, `$C1CC70` stores its `+8` field, `$C1EF10` publishes the cursor, and `$C1F70E` loads that exact published value into `A5`. `A1` at walker entry is a separate cursor, not the stream pointer."
         if same_trace else
