...
The $C1CB74 selector loop uses $C4E9AA as an anchor.  In the captured
runtime states the first aligned 24-byte record begins two bytes later, at
$C4E9AC: a descriptor pointer followed by four word fields and twelve bytes
of per-frame state.  This utility preserves that distinction and emits a
...
def records(path: Path) -> list[dict]:
    ...
    for index in range(MAX_RECORDS):
        ...
        words = struct.unpack_from(">10H", raw, offset + 4)
        ...
        if not (0xC22000 <= descriptor < 0xC23000 and words[1] == 0):
            continue
        rows.append({
            "index": index,
...
def main() -> None:
    ...
    payload = {"authority": {"selector_loop": "analysis/routines/c1cb14_flight_update_stage.md",
                              "runtime_anchor": "$C4E9AA", "first_aligned_record": "$C4E9AC"},
    ...
    lines = ["# Runtime coordinate-bearing scene placements", "",
             ...
             "| Bytes | Observed role |", "| --- | --- |", "| `+0..3` | relocated descriptor pointer in `$C22000-$C22FFF` |", "| `+4..9` | three signed coordinate-bearing words; the middle word is zero in every exported record |", "| `+10..11` | additional record field (role unknown) |", "| `+12..23` | mutable per-frame words |", "",
