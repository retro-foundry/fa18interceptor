...
def descriptor_control_stream_field(memory: bytes, descriptor: int) -> int | None:
    """Read descriptor +8 only when it belongs to the captured slow-RAM bank.

    The generic placement route copies this field to C45A36, but descriptor
    type gates can take another route.  Preserve it as a raw field value rather
    than claiming every record submits this stream to the renderer.
    """
    offset = descriptor - SLOW_BASE + 8
    if not 0 <= offset <= len(memory) - 4:
        return None
    return int.from_bytes(memory[offset:offset + 4], "big")
...
def inventory() -> list[dict]:
    ...
    for row in trace:
        ...
        rows.append({
            ...
            "descriptor_plus_8_field": (
                hex_address(descriptor_control_stream_field(memory, consumer["descriptor"]))
                if consumer and descriptor_control_stream_field(memory, consumer["descriptor"]) is not None
                else None),
...
def markdown(rows: list[dict]) -> str:
    ...
    for row in rows:
        ...
        if row["runtime_placement_record"] is None:
            ...
            emitted = (f"{row['runtime_placement_record']} / {row['runtime_descriptor']} / "
                       f"{row['descriptor_plus_8_field'] or 'outside captured slow RAM'} / "
                       f"({words})")
        lines.append(
    ...
    lines += [
        ...
        "descriptor `+8` field, and three signed emitted placement words. The generic route "
        "uses that field as `$C45A36`, but type-specific gates can bypass it. These words are generated by the builder, "
