...
def main() -> None:
    ...
    targets = sorted({cell["target"] for cell in cells
                      if cell["entry_classification"] != "immediate_C2AEFC_reject"})
    ...
    draw.text((18, 36), "grey = C2AEFC immediate reject; colour = non-reject static target; white outline = traced access", fill="#cbd5e1")
    ...
    for y in range(8):
        ...
        for x in range(8):
            ...
            reject = cell["entry_classification"] == "immediate_C2AEFC_reject"
            draw.rectangle((left, top, left + cell_size - 3, top + cell_size - 3),
                           fill="#d9d9d9" if reject else colours[cell["target"]],
                           outline="#ffffff" if (x, y) in used else "#18212a", width=4 if (x, y) in used else 1)
            draw.text((left + 8, top + 34), cell["target"], fill="#10151c")
            if reject:
                draw.text((left + 8, top + 54), "REJECT", fill="#10151c")
