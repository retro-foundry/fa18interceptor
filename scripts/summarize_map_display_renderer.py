...
def write_report(output: Path, report: dict[str, object]) -> None:
    ...
    for row in report["control_to_primitive"]:
        lines.append(
            f"| {row['frame']} | `{row['transform_input']}` | `{row['raw_triples']}` | `{row['control_entry']}` | `{row['line_submissions']}` | "
...
def main() -> None:
    ...
    for item, index in enumerate(control_indices):
        ...
        line_submissions = [
            {"context_a5": hex_address(entry["registers"]["a5"]),
             "endpoints": [entry["registers"][f"d{component}"] & 0xFFFF
                           for component in range(4)]}
            for entry in interval if entry["pc"] == LINE_EMIT]
        line_endpoints = [submission["endpoints"] for submission in line_submissions]
        polygon_routes = []
        ...
        control_to_primitive.append({
            ...
            "line_submissions": line_submissions,
